...
NEW_RANDOM_FOREST_MODEL_ONLY_FEATURES_PATH = (
    "./models/rf_model_new_labels_features_only.pkl"
)
NEW_BAYESIAN_MODEL_ONLY_FEATURES_PATH = (
    "./models/naive_bayes_model_new_labels_features_only.pkl"
)
...
def main():
    ...
    print(
        "This project aims to classify constructive comments from a subset of the SFU Opinion and Comments Corpus.\n"
    )
    print(
        "As constructiveness is highly subjective and the annotations were done by crowdworkers, you have the option to:"
    )
    print("  1. Use the old annotations (done by crowdworkers).\n")
    print(
        "  2. Use the new annotations (generated using the following set of rules: at least 300 characters, 10 verbs and 8 adjectives).\n"
    )
    print(
        "  3. Use the new annotations (generated using the following set of rules: at least 300 characters, 10 verbs, 8 adjectives, and the ratio of constructive/non-constructive keywords.) "
    )
    ...
    if user_choice == "1":
        ...
        test_models(
            annotations, original_texts, df_features, new_NB, new_RF, user_choice
        )
...
def test_models(annotations, original_texts, df_features, NB, RF, user_choice):
    ...
    if user_choice == "1":
        ...
        print(
            "The feature based classifier has really good result, it would be really intersting to look into how it takes decision, for this, SHAP values are a really intersting tool"
        )
        print("=" * 40)
        print(
        ...
        print(
            "2. Features with large values (e.g., many nouns or verbs) increase the likelihood of predicting class 1: Constructive comments."
        )
        print(
            "   - Exception: More sentences in a comment increase the likelihood of being predicted as Not-Constructive, but the SHAP value for this is still very low."
        )
        print(
            "3. Conjunctions, pronouns, and adverbs don't seem to play a significant role in our model's decision-making process."
        )
        print("\n" + "=" * 40)
        ...
        print(
            "Following the SHAP Values Analysis, we decided to create our own definiton of constructive comments, with the defined set of rules for our new annotations"
        )
    else:
        print(
            "Rule based annotations improve the overall performance of the bayesian classifier, however the performance to predict the class 1 (constructive) is quite bad."
        )
        print(
            "It also important to point out that it is not necessary to build feature based model, as the rules are already clearly defined."
        )
        print(
            "We can aswell see that with the selected rules, we have more comments falling under the non-constructive class."
        )
        print(
            "It doesn't seem to surprising as we could expect online comments to be mostly short and less detailed than a real conversation or debate for example"
        )
